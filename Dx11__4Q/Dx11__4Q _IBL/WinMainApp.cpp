...
void WinMainApp::Render()
{
	...
	SB.useCustomMetal = m_bUseCustomMetal ? 1.0f : 0;
	SB.CustomMetal = m_fCustomMetal;
	...
	if (useIBL)
	{
		...
		m_pDeviceContext->IASetVertexBuffers(0, 1, &m_pModelSky->m_pVertexBuffer, &m_pModelSky->stride, &m_pModelSky->offset);
		m_pDeviceContext->IASetIndexBuffer(m_pModelSky->m_pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);
		m_pDeviceContext->VSSetShader(m_pSkyVertexShader, nullptr, 0); //정점 셰이더 설정
		m_pDeviceContext->PSSetShader(m_pSkyPixelShader, nullptr, 0);//픽셀 셰이더 설정
		m_pDeviceContext->PSSetShaderResources(0, 1, &m_envTexture);
		m_pDeviceContext->PSSetSamplers(0, 1, &m_pDefaultSamplerState);
		m_pDeviceContext->OMSetDepthStencilState(m_pSkyDepthstencilState, 0);
	...
	if (m_bShowImGUI)
	{
		...
		ImGui::Text("Light");
		ImGui::SliderFloat3("light0 dir", &Light1Direction.x, -1.0f, 1.0f);
		ImGui::SliderFloat3("light0 rad", &Light1Radiance.x, -1.0f, 1.0f);
		ImGui::SliderFloat3("light1 dir", &Light2Direction.x, -1.0f, 1.0f);
		ImGui::SliderFloat3("light1 rad", &Light2Radiance.x, -1.0f, 1.0f);
		ImGui::SliderFloat3("light2 dir", &Light3Direction.x, -1.0f, 1.0f);
		ImGui::SliderFloat3("light2 rad", &Light3Radiance.x, -1.0f, 1.0f);

		ImGui::Text("");

		ImGui::Checkbox("useIBL", &useIBL);

		ImGui::Checkbox("useCustomMetal", &m_bUseCustomMetal);
		ImGui::SliderFloat("CustomMetal", &m_fCustomMetal, 0.0f, 1.0f);

		ImGui::Checkbox("useCustomRough", &m_bUseCustomRough);
		ImGui::SliderFloat("m_fCustomRough", &m_fCustomRough, 0.1f, 1.0f);
...
bool WinMainApp::InitD3D()
{
	...
	HR_T(m_pDevice->CreateDepthStencilState(&DSDesc, &m_pDefaultDepthstencilState));

	DSDesc.DepthEnable = false;
	DSDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	HR_T(m_pDevice->CreateDepthStencilState(&DSDesc, &m_pSkyDepthstencilState));
...
bool WinMainApp::InitScene()
{
	...
	HR_T(CompileShaderFromFile(L"Shaders/skybox_vs.hlsl", "main_vs", "vs_5_0", &pVertexShaderBlob));

	HR_T(m_pDevice->CreateInputLayout(skyboxLayout, ARRAYSIZE(skyboxLayout), pVertexShaderBlob->GetBufferPointer(), pVertexShaderBlob->GetBufferSize(), &m_pSkyLayout));
	// 정점 셰이더 생성
	HR_T(m_pDevice->CreateVertexShader(pVertexShaderBlob->GetBufferPointer(), pVertexShaderBlob->GetBufferSize(), NULL, &m_pSkyVertexShader));
	SAFE_RELEASE(pVertexShaderBlob);
	...
	m_pSky = Mesh::fromFile("Resources/skybox.obj");
	m_pModelSky = new Model;
	m_pModelSky->createMeshBuffer(m_pDevice, m_pSky);
	...
	m_envTexture = createShaderResourceViewDDS("Resources/indoorEnvHDR.dds");
	m_irmapTexture = createShaderResourceViewDDS("Resources/indoorDiffuseHDR.dds");
	m_SpecularTexture = createShaderResourceViewDDS("Resources/indoorSpecularHDR.dds");
	m_spBRDF_LUT = createShaderResourceViewDDS("Resources/indoorBrdf.dds");
...
ID3D11ShaderResourceView* WinMainApp::createShaderResourceViewDDS(const std::string& path) const
{
	ID3D11ShaderResourceView* texture;

	std::wstring wfilename = std::wstring(path.begin(), path.end());

	if (FAILED(DirectX::CreateDDSTextureFromFile(m_pDevice, wfilename.c_str(), nullptr, &texture))) {
		throw std::runtime_error("Failed to create cubemap texture SRV");
	}
	return texture;
}
