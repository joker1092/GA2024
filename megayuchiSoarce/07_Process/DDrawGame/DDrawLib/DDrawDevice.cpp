...
void CDDrawDevice::Clear()
{
	...
	{
		memset(m_pSendBuffer + y * m_dwWriteBufferPitch, 0, 4 * m_dwWidth);
	}
}
...
void CDDrawDevice::DrawRect(int sx, int sy, int iWidth, int iHeight, DWORD dwColor)
{
	...
	for (int y = start_y; y < end_y; y++)
	{
		...
		{
			DWORD*	pDest = (DWORD*)(m_pSendBuffer + x * 4 + y * m_dwWriteBufferPitch);
			*pDest = dwColor;
		}
	}
}
...
BOOL CDDrawDevice::BeginDraw(CSharedMemoryWriter* pMemoryWriter)
{
	...
	if (pMemoryWriter)
	{
		m_pSendBuffer = m_pScratchBuffer;
...
void CDDrawDevice::EndDraw()
{
	if (m_pCurMemoryWriter)
	{
		...
		m_pCurMemoryWriter->Write(m_pSendBuffer, m_dwWidth, m_dwHeight, m_dwWriteBufferPitch);
	}
	else
	{
		UnlockBackBuffer();
...
BOOL CDDrawDevice::DrawBitmap(int sx, int sy, int iBitmapWidth, int iBitmapHeight, char* pBits)
{
	...
	char* pDest = m_pSendBuffer + (ivDestStart.x * 4) + ivDestStart.y * m_dwWriteBufferPitch;
...
BOOL CDDrawDevice::DrawImageData(int sx, int sy, const CImageData* pImgData)
{
	...
	char* pDestPerLine = m_pSendBuffer + (ivDestStart.y) * m_dwWriteBufferPitch;
