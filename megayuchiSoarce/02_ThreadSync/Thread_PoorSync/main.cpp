...
int main()
{
	...
	{	
		...
		while (!bCompleted)
		{
			...
			for (DWORD i = 0; i < dwWorkerThreadCount; i++)
			{
				dwCompletedThreadCount += (pArgList[i].lCompleted != 0);	 	// 이 시점에서 worker스레드가 lCompleted변수에 써넣으려고 할 수 있다. 일단 4bytes align되어있다면 큰 문제는 아니다.
...
UINT __stdcall ThreadFunc(void* pArg)
{
	...
	while (pThreadArg->lAlive)
	{
		if (pThreadArg->lStart)
		{
			...
			pThreadArg->lCompleted = 1;	// 이 시점에 메인스레드에서 pThreadArg->lCompleted의 값을 읽으려 할 수도 있다. 4 bytes align되어있다면 일단 큰 문제는 아니다.
			
		}
		if (pThreadArg->lFinish)	// 이 시점에 메인스레드에서 lFinish 값을 써넣고 있을 수 있다. 4 bytes align되어있다면 일단 큰 문제는 아니다.
		{
			break;
		}
	}
